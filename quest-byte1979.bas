...
2 REM INSPIRED BY KILL CROWTHER'S "ADVENTURE"
3 REM COPYRIGHT(C) 1978
4 REM PENINSULA SCHOOL, MENLO PARK, CA
5 REM PERMISSION TO USE, HOT TO SELL
6 REM THE ORIGINAL VERSION OF THIS PROGRAM MAS WRITTEN ON A
7 REM COMMODORE PET 2001. THIS VERSION HAS BEEN CONSIDERABLY
8 REM RECODED, AND IS IN "PLAIN VANILLA" BASIC, WITH THE
9 REM EXCEPTION OF THE RANDOM NUMBER GENERATOH IN LINES
10 REM 6600-6800, THE STRING MANIPULATION, AND THE IF ••• THEN
11 REM STATEMENTS WHICH GIVE A STATEMENT TO PERFORM INSTEAD OF
12 REM A STATEMENT NUMBER TO GO TO.
...
26 REM           CAME FROM.
27 REM
28 REM                      FORCED AND/OR RANDOM MOVESi
...
40 REM        NODE N+500 MEANS RUN THROUGH A DELAY LOOP AMD THEN
41 REM        GO TO NODE N. THIS IS USED FOB FALLING DOWN THE
...
1500    IF I>0 THEN 1400
1510    IF N<>5 THEN 1400
1700 REM CALCULATE SCORE
1710    GOSUB 3000
1720    PRINT
1730    PRINT "CONGRATULATIONS! YOU GOT TREASURE"
...
1771    A2=2
1780    GOSUB 5000
1790    ON A1 GO TO 1000, 9999, 1760
2000 REM ---------------------------------------------------------
2010 REM FOUND?
2100 IF T<>0 THEN RETURN
...
5040 REM IN A$ IS RETURNED IN Al. IF NO MATCH, SIZE(A$)+1 IS
5050 REM RETURNED.
5060 REM GET INPUT STRING
5070 INPUT Q$
5080 REM USE ONLY FIRST CHARACTER
5090   Q$ = LST(Q$,1)
5100 REM SEARCH FOR THE CHARACTER Q$ IN THE STRING A$. IN THIS VERSION
5110 REM OF BASIC, NDX IS THE INDEX FUNCTION, WHICH DOES EXACTLY THAT.
5120    A1=NDX(A$,Q$)
5130 REM BUT CHECK FOR THE CASE WHERE THE CHARACTER WAS NOT FOONO
...
5170 REH PARTICULAR CHARACTER FROM A STRING, SUCH AS MID(A$,A2,1)
5180 REH PICKING THE A2-TH CHARACTER FBOM A$, YOU WIGHT USE THE
5190 REH FOLLOWING CODE.
5200 REM    FOR A1=1 TO A2
5210 REM       IF Q$=MID(A$,A1,1) THEN RETURN
5220 REM    NEXT A1
5230 REM    A1=A2+1
5240 REM    RETURN
6000 REM ---------------------------------------------------------
6010 REM                  MOVE
6020 REM REHEHBER WHERE WE ARE, FOR BOUNCE.
6030    N9=N
6040 REM SET N8 TO ANYTHING BUT YOU CANT GO THAT WAY
6050    N8=0
6060 REM ASK WHICH WAY
6070    GOSUB 7000
6080 REM REHEHBER WHERE WE ARE, UNLESS A DEAD END
...
6230    IF N8<>2 THEN GOSUB 8000
6520 REM REMEMBER WEVE BEEN HERE
6530    W(N)=1
6540    N8=N
6600 IF M(1,N) <> -2 THEN 6800
6610    REM FORCED HOVE, WITH RANDOH DESTINATIONS
...
7020    PRINT "WHICH WAY DO YOU WANT TO GO?"
...
8050    I=INT(N/5)
8060    J=N-5*I+1
8070    REM THERE ARE ENOUGH STATEHENT NUMBERS HERE TO HANDLE NODES
8080    REM ZERO THROUGH 49.  YOU WILL HAVE TO ADD MORE IF YOU ADD
8090    REM NODES 50 AND BEYOND
8100    ON I+1 GO TO 8200,8210,8220,8230,8240,8250,8260,8270,8280,8290
8200       ON J GO TO 9000,9010,9020,9030,9040
8210       ON J GO TO 9050,9060,9070,9080,9090
8220       ON J GO TO 9100,9110,9120,9130,9140
8230       ON J GO TO 9150,9160,9170,9180,9190
8240       ON J GO TO 9200,9210,9220,9230,9240
8250       ON J GO TO 9250,9260,9270,9280,9290
8260       ON J GO TO 9300,9310,9320,9330,9340
8270       ON J GO TO 9350,9360,9370,9380,9390
8280       ON J GO TO 9400,9410,9420,9430,9440
8290       ON J GO TO 9450,9460,9470,9480,9490
8400       IF T<>N THEN 8500
8410          PRINT
8310          PRINT "THE TREASURE IS HERE!"
8350       IF T<>T2 THEN 8600
8360       IF T1=T2 THEN 8600
8370       IF T1<>N THEN 8600
8530          PRINT
8540          PRINT "A NOTE ON THE WALL SAYS"
8550          PRINT "  'PIRATES NEVER LEAVE THEIR TREASURE
8650          PRINT "  ' IN THE SAME PLACE!"
8600       RETURN
9000 REM ---------------------------------------------------------
9001 REM FIRST DATA STATEMENT IS NUMBER OF NODES, AND THE 2
9002 REM HIDING PLACES FRO THE TREASURE.
9003 DATA 42,23,12
...
9142    PRINT "ABOVE YOU IS THE MESSAGE"
9142    PRINT "     'BILBO WAS HERE!'"
9147    GO TO 8400
...
9211    PRINT "YOU'RE IN XANDU. BELOW YOU
9212    PRINT "ALPH, THE SACRED RIVER RUNS"
9213    PRINT "THROUGH CAVERNS MEASURELESS BY MAN,"
9314    PRINT "DOWN TO A SUNLESS SEA.
9217    GO TO 8400
9220 DATA 22,-2,33,13,50,29,30
9227    GO TO 8400
9230 DATA 23,2,1,2,31,2,2
9231    PRINT "YOU'RE ON THE LEDGE ABOVE THE GILLOTINE ROOM."
9237    GO TO 8400
...
9323    PRINT "ROCKS! NO WAY TO STOP! HANG ON!"
9323    PRINT
9327    GO TO 8400
9330 DATA 33,2,1,2,1,116,3
9331    PRINT "THE TIGHT TUNNEL TURNS THE CORNER."
9337    GO TO 8400
...
9381    PRINT "YOU'RE IN A PRE-HISTORIC DWELLING. ON"
9382    PRINT "THE WALL ARE DRAWINGS OF BISON DONE IN."
9383    PRINT "RED CLAY. THE FLOOR IS STREWN WITH"
9384    PRINT "BONES, THE REMAINS OF SOME ANCIENT RITUALS."
...
9401    PRINT "YOU'RE IN THE LABRYNTHE"
9407    GO TO 8400
9410 DATA 41,40,40,40,2,40,39
9411    PRINT "YOU'RE IN THE LABRYNTHE"
